...
import { createHouse } from "@/lib/actions/master.actions";
...
export default function CreateHouseForm() {
    // @ts-ignore
    const [state, formAction] = useFormState(createHouse, initialState);

    return (
        <form action={formAction} className="bg-white p-6 rounded shadow space-y-4">
            <h3 className="text-lg font-medium leading-6 text-gray-900 mb-2">New House</h3>
            ...
            <div>
                ...
                <input name="name" placeholder="Red House" required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2 text-sm" />
            </div>

            <div>
                <label className="block text-sm font-medium text-gray-700">Color (Hex)</label>
                <input name="color" placeholder="#FF0000" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2 text-sm" />
            </div>
